...
class QuintonMaster(CplexModel):
    ...
    def setup_variables(self):
        ...
        self.decision_vars.K = mdl.integer_var_dict([(a1, a2) for a1 in self.data.TASKS for a2 in self.data.TASKS if a1 != a2], name='K', lb=-mdl.infinity, ub=1e9)  # Repetition distance between two tasks
    ...
    def setup_constraints(self, data):
        ...
        for i in self.data.TASKS:
            for r in self.data.TASKS[i]:
                ...
                mdl.add(M * (1 - m[(i,r)]) + w[c[1]] + self.data.DEPENDENCIES[c] >= w[c[0]] + T_lb * self.data.TASKS[i][r] for c in self.data.DEPENDENCIES if c[0] == i)
                # Constraint (8b) in Quinton et al. 2020
                mdl.add(M*(2 - m[(i,r2)] - m[(ij[1],r2)]) + w[ij[1]] + K[ij] >= w[ij[0]] + T_lb*self.data.TASKS[i][r]
