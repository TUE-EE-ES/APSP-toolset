...
class QuintonMagnantiWongSub(CplexModel):
    ...
    def setup_constraints(self, *data):
        ...
        mdl.add(
            (mdl.sum(pi_nr[i] * mdl.sum(m_bar[(i, r)] / self.data.TASKS[i][r] for r in self.data.TASKS[i]) for i in
                     self.data.TASKS) +
             mdl.sum(
                pi_d[(ij[0], ij[1], r)] * (M * (2 - m_bar[(ij[0], r)] - m_bar[(ij[1], r)]) + K_bar[(ij[0], ij[1])]) for
        ...
        mdl.minimize(
            ...
            mdl.sum(pi_d[(ij[0], ij[1], r)]*(M*(2 - m_core[(ij[0], r)] - m_core[(ij[1], r)]) + K_core[(ij[0], ij[1])]) for ij in self.data.ALLOCATION_OVERLAP.keys() for r in self.data.ALLOCATION_OVERLAP[ij]) +
            mdl.sum(pi_p[c] * self.data.DEPENDENCIES[c] for c in self.data.DEPENDENCIES) +
