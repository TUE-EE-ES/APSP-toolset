...
class BoundedQuintonDualSub(CplexModel):
    ...
    def setup_constraints(self, *data):
        ...
        mdl.minimize(
            ...
            mdl.sum(pi_d[(ij[0], ij[1], r)]*(P1*(2 - m_bar[(ij[0], r)] - m_bar[(ij[1], r)]) + K_bar[(ij[0], ij[1])]) for ij in self.data.ALLOCATION_OVERLAP.keys() for r in self.data.ALLOCATION_OVERLAP[ij]) +
