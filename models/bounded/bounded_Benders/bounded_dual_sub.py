...
class BoundedDualSub(CplexModel):
    ...
    def setup_constraints(self, *data):
        ...
        # Constraint (32)
        mdl.maximize(
            mdl.sum(pi_p[(c[0], self.data.DEPENDENCIES[c], c[1])] * mdl.sum(x_bar[(c[0], p)] * self.data.TASKS[c[0]][p] for p in self.data.TASKS[c[0]]) for c in self.data.DEPENDENCIES) +
            mdl.sum(pi_nl[(ij[0], ij[1], p, m)] * (self.data.TASKS[ij[0]][p] - M * (3 - x_bar[(ij[0], p)] - x_bar[(ij[1], p)] - y_bar[(ij[0], ij[1], m)]))
                    for ij in self.data.ALLOCATION_OVERLAP.keys()
                    for p in self.data.ALLOCATION_OVERLAP[ij]
                    for m in range(self.data.BOUND + 1)) +
            mdl.sum(pi_nr[(ij[0], ij[1], p, m)] * (self.data.TASKS[ij[1]][p] - M * (2 - x_bar[(ij[0], p)] - x_bar[(ij[1], p)] + y_bar[(ij[0], ij[1], m)]))
                    for ij in self.data.ALLOCATION_OVERLAP.keys()
                    for p in self.data.ALLOCATION_OVERLAP[ij]
                    for m in range(self.data.BOUND + 1)) +
            mdl.sum(pi_nd[a] * mdl.sum(x_bar[(a,p)] * self.data.TASKS[a][p] for p in self.data.TASKS[a]) for a in self.data.TASKS if 0 not in self.data.TASKS[a].values())
        )
